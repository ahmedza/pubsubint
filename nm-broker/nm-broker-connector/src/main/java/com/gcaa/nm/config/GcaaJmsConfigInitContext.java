...
public class GcaaJmsConfigInitContext {
	...
	public InitialContext initialContext() throws NamingException {
		...
		try {
			...
			/*
			 * env.put("connectionfactory.nmConnectionfactory",
			 * nmBrokerEndPointUrl+"?transport.keyStoreLocation="+keyStorePath+
			 * "&transport.keyStorePassword="+keyStorePswd+
			 * "&transport.trustStoreLocation="+trustStorePath+
			 * "&transport.trustStorePassword="+trustStorePswd+
			 * "&transport.trustAll=false&transport.verifyHost=true&transport.keyAlias=1"
			 * );
			 */
			env.put("connectionfactory.nmConnectionfactory", nmBrokerEndPointUrl);
			env.put("connectionfactory.gcaaConnectionfactory",
					gcaaBrokerUrl + "?jms.username=" + gcaaBrokerUser + "&jms.password=" + gcaaBrokerPswd);
	...
	public JmsTemplate gcaaJmsTemplate(InitialContext initialContext) throws NamingException, JMSException {
		...
		/*
		 * if(null != gcaaMsgTimeToLive && !gcaaMsgTimeToLive.isEmpty()){
		 * template.setTimeToLive(Long.parseLong(gcaaMsgTimeToLive)); }else{
		 */
		// setting default time to live for producer
		template.setTimeToLive(21600000); // Message lives for 6 hours in queue
		/* } */
